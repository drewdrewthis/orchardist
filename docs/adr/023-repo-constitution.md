...
PR #610 (sidebar redesign) shipped with bundled work spanning 5 domains: ADR-022 pane-first refactor, batched PR enrichment, `sendTextToPane` mutation, PWA shell, mobile layout. The post-merge `/review` surfaced 16 concerns — most of them cross-cutting patterns that no PR-level review had caught:

- **Snapshot() in field resolvers** caused ~60s cold lens loads (#612)
- **Duplicate code paths** for single-vs-batch enrichment in `gh` (#615)
- **Half-removed subsystems** (chat) hydrating on boot with no UI (#616)
...
2. **[RULES.md](../../RULES.md)** — the 61-rule constitution: 10 layer rules (L1–L10), 16 code patterns (R1–R16), 16 schema rules (S1–S16), 12 optimization rules (O1–O12), 7 mutation-coverage rules (M1–M7). Rules are numbered with stable IDs for citation.
...
- The daemon module refactor (#613) has a concrete spec: bring each domain up to constitution conformance. The 61 rules ARE the acceptance criteria.
...
- 61 rules is a lot to internalize. Mitigated by stable IDs and category structure: most PRs only touch ~5 relevant rules at a time.
...
1. **Document patterns informally in CLAUDE.md.** Rejected: CLAUDE.md is loaded into every Claude session — adding 61 rules bloats every conversation's context. Rules belong in `RULES.md` (loaded on demand), cited from CLAUDE.md.
...
4. **Skip the audit checklist; just refactor.** Rejected — see #610's `/review` findings. Without a written rubric, "refactor" is unbounded.

## References

- [docs/architecture.md](../architecture.md) — the ecosystem and domain descriptions
- [RULES.md](../../RULES.md) — the 61-rule constitution
