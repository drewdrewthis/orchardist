...
PR #730 (sidebar redesign) shipped with bundled work spanning 5 domains: ADR-022 pane-first refactor, batched PR enrichment, `sendTextToPane` mutation, PWA shell, mobile layout. The post-merge `/review` surfaced 16 concerns — most of them cross-cutting patterns that no PR-level review had caught:

- **Snapshot() in field resolvers** caused ~60s cold lens loads (#732)
- **Duplicate code paths** for single-vs-batch enrichment in `gh` (#735)
- **Half-removed subsystems** (chat) hydrating on boot with no UI (#736)
...
2. **[RULES.md](../../RULES.md)** — the 73-rule constitution: 11 layer rules (L1–L11), 17 code patterns (R1–R17), 19 schema rules (S1–S14 + S15a/b + S16a/b/c), 12 optimization rules (O1–O12), 7 mutation-coverage rules (M1–M7), 7 testing rules (T1–T7). Rules are numbered with stable IDs for citation.
...
- The daemon module refactor (#733) has a concrete spec: bring each domain up to constitution conformance. The 73 rules ARE the acceptance criteria.
...
- 73 rules is a lot to internalize. Mitigated by stable IDs and category structure: most PRs only touch ~5 relevant rules at a time.
- The script-as-canonical pattern requires extracting current inline logic into standalone scripts. That's its own refactor, parallel to #733.
...
1. **Document patterns informally in CLAUDE.md.** Rejected: CLAUDE.md is loaded into every Claude session — adding 73 rules bloats every conversation's context. Rules belong in `RULES.md` (loaded on demand), cited from CLAUDE.md.
...
4. **Skip the audit checklist; just refactor.** Rejected — see #730's `/review` findings. Without a written rubric, "refactor" is unbounded.

## References

- [docs/architecture.md](../architecture.md) — the ecosystem and domain descriptions
- [RULES.md](../../RULES.md) — the 73-rule constitution
- #733 — daemon module refactor (the first major application of these rules)
- #730 — sidebar redesign PR; `/review` of it surfaced the patterns this ADR codifies
