...
Worktree (the durable workspace)
  ...
  │     ├── Merge conflicts
  │     └── Open comments count
  ├── Branch name → Issue number (by convention: `webapp-2478`)
...
2. The issue number drives naming: branch `webapp-2478`, worktree `.worktrees/webapp-2478`
...
Each repo has a persistent `repo_main` tmux session (e.g., `webapp_main`) at the repo root. Auto-created on startup, always exists, pinned at the top of the dashboard.
