...
6. Optionally, match **branch name to issue number** by naming convention (e.g., `langwatch-2478` → issue #2478). Look up issue title and state from `{owner}_{repo}_issues.json`.
...
{
  "sessions": [
    { "name": "langwatch_langwatch-2478", "path": "/path/to/worktree", "had_claude": true },
    { "name": "langwatch_main", "path": "/path/to/repo", "had_claude": false }
  ]
}
```
