#!/usr/bin/env bash
# check-feature-parity-gui.sh — verify every GUI scenario has a matching
# @scenario annotation in crates/orchard-gui/**/*.{test.ts,spec.ts} files.
#
# Exit codes:
...
FEATURES_DIR="$SCRIPT_DIR/../crates/orchard-gui/features"
TESTS_DIR="$SCRIPT_DIR/../crates/orchard-gui"
JSON_MODE=false
...
# ---------------------------------------------------------------------------
# 2. Extract @scenario annotations from TypeScript test files.
#    Both forms: // @scenario <title>  and  /** @scenario <title> */
# ---------------------------------------------------------------------------
...
done < <(find "$TESTS_DIR" \( -name "*.test.ts" -o -name "*.spec.ts" \) -print0 2>/dev/null || true)
...
if [[ "$JSON_MODE" == "true" ]]; then
  ...
  echo "=== GUI (crates/orchard-gui) feature parity check ==="
  echo "  scenarios:   $scenario_count"
  echo "  annotations: $annotation_count"
  echo ""

  if [ "$unbound_count" -gt 0 ]; then
    echo "GAPS (follow-up work): $unbound_count GUI scenario(s) need TypeScript test implementations:"
    for title in "${unbound_scenarios[@]}"; do
      echo "  - $title"
    done
    echo ""
    echo "  Track with: gh issue create --title 'feat(gui): implement @scenario tests for GUI feature parity'"
    echo ""
  fi

  if [ "$unknown_count" -gt 0 ]; then
    echo "FAIL: $unknown_count annotation(s) do not match any GUI scenario:"
  ...
  if [[ "$has_stale" == "false" && "$unbound_count" -eq 0 ]]; then
    echo "PASS: all $scenario_count GUI scenarios are bound."
  elif [[ "$has_stale" == "false" ]]; then
    echo "STATUS: $unbound_count GUI scenarios need implementation (see above)."
  fi
fi
