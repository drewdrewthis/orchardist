...
func TestGitProvider_E2E(t *testing.T) {
	...
	resolver.Git = provider
	resolver.ProjectsSrc = &resolvers.StaticProjectsSource{
		Records: []resolvers.ProjectRecord{
			{ID: projectID, Directory: repo, Name: "demo"},
		},
	}

	srv := server.NewWithResolver("", nil, resolver)
