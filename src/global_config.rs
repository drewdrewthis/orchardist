...
mod tests {
    ...
    fn repo_config_owner_and_name_parsing() {
        let repo = RepoConfig {
            slug: "langwatch/langwatch".to_string(),
            path: "/workspace/langwatch".to_string(),
            remotes: vec![],
        };

        assert_eq!(repo.owner(), "langwatch");
        assert_eq!(repo.repo_name(), "langwatch");
    }
    ...
    fn config_with_remotes_array_using_repo_path() {
            ...
            "repos": [
                {
                    ...
                    "path": "/workspace/repo",
                    "remotes": [
                        { "name": "remmy", "host": "ubuntu@10.0.3.56", "repoPath": "~/langwatch-workspace/langwatch-bare", "shell": "mosh" }
                    ]
                }
            ]
        }"#;
        ...
        assert_eq!(cfg.repos[0].remotes.len(), 1);
        assert_eq!(cfg.repos[0].remotes[0].path, "~/langwatch-workspace/langwatch-bare");
    }
    ...
    fn repo_config_owner_with_different_owner_and_name() {
        let repo = RepoConfig {
            slug: "hopegrace/git-orchard-rs".to_string(),
            path: "/workspace/git-orchard-rs".to_string(),
            remotes: vec![],
        };

        assert_eq!(repo.owner(), "hopegrace");
    ...
    fn legacy_remotes_array_parses_correctly() {
        ...
        let json = r#"{"remotes":[{"name":"remmy","host":"ubuntu@10.0.3.56","repoPath":"~/langwatch-workspace/langwatch-bare","shell":"mosh"}]}"#;
        let raw: RawOrchardJson = serde_json::from_str(json).unwrap();

        assert!(raw.remote.is_none());
        let remotes = raw.remotes.unwrap();
        assert_eq!(remotes.len(), 1);
        assert_eq!(remotes[0].host.as_deref(), Some("ubuntu@10.0.3.56"));
        assert_eq!(
            remotes[0].repo_path.as_deref(),
            Some("~/langwatch-workspace/langwatch-bare")
        );
