...
/// Derives the repo name from a slug like "acme/my-project" -> "git-orchard-rs".
...
mod tests {
    ...
    fn derive_repo_name_from_slug() {
        assert_eq!(repo_name_from_slug("acme/my-project"), "git-orchard-rs");
    }
    ...
    fn sync_returns_false_with_no_issues() {
        let mut state = empty_state();
        let modified = sync_issues_with_data(&mut state, "acme/my-project", &[]);
        assert!(!modified);
