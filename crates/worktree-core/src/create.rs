...
//!
//! Creates a worktree at `path` for `branch`. Tries `git worktree add -b
//! <branch> <path>` first (creates a new branch); on failure falls back to
//! `git worktree add <path> <branch>` (checks out an existing branch).
//!
//! This function is git-only. Tmux session creation, setup-script execution,
//! and remote dispatch are concerns of the consuming binary.
...
/// checked out. Callers that want to surface this to the user (e.g., "created
/// branch X" vs "checked out existing branch X") branch on this.
...
/// # Errors
///
/// Returns `Err` if both attempts fail or if the second attempt produces
/// stderr output (surfaced verbatim to the caller).
